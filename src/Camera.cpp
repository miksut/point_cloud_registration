...
namespace cgCourse
{
    ...
    void Camera::arcballRotate(glm::vec2 _p1, glm::vec2 _p2){
       ...
       // float temp_sign = glm::sign(axis.x);
        ...
        //right = glm::cross(newv,newup);
/*
        double currentAngle =  glm::dot(glm::normalize(newv), glm::vec3(0,1,0));
        float newAngle = currentAngle - angle * glm::dot(axis,right);
        newAngle = glm::clamp(newAngle, -1.f, 1.f);
        if (currentAngle != newAngle){
            glm::vec3 projectedV = newv;
            projectedV.y = 0;
  ...
  */
       ...
       // std::cout<<axis.x<<" "<<axis.y<<" "<<axis.z<<std::endl;
        axis=glm::normalize(axis);
        if (angle !=1 && glm::length(axis)>0.1){
            glm::mat3 rotMat = glm::rotate(glm::mat4(1), acos(glm::clamp(abs(angle),0.0f,1.0f)), axis);
          //  std::cout<<"---"<<-acos(abs(angle))<<" "<<rotMat[0][0]<<" "<<rotMat[0][1]<<" "<<rotMat[0][2]<<std::endl;
        auto vector = position - target;
        vector = rotMat * vector;
           // std::cout<<vector.x<<" "<<vector.y<<" "<<vector.z<<std::endl;
        position = target + vector;
          //  axis.y = 0;
          //  rotMat = glm::rotate(glm::mat4(1), acos(glm::clamp(abs(angle),0.0f,1.0f)), axis);
            glm::vec3 t,tt;
            //t= glm::cross(vector,up);
            
            up = rotMat * up;
            up = glm::normalize(up);
            vector = glm::normalize(vector);
            glm::vec3 vector_proj = vector;
            vector_proj.y=0;
            

            vector_proj = glm::normalize(vector_proj);
            t =glm::cross(vector,glm::vec3(0,1,0));
            tt =glm::sign(vector.y)* glm::cross(vector,vector_proj);
            if (glm::length(tt)>glm::length(t)){
                t = tt;
                std::cout<<"up";
            }else{
                //t =
        ...
        ////////
        /*
        cp2 = glm::vec2(-_p1.x,_p2.y) - center;
        
      //  cp1 = cp1 / r;
        cp2 = cp2 /r;
        z1sq = 1- cp1.x*cp1.x - cp1.y*cp1.y;
        z2sq = 1- cp2.x*cp2.x - cp2.y*cp2.y;
        
        //glm::vec3 p1_z,p2_z;
        if (z1sq>0){
            p1_z = glm::vec3(cp1.x,cp1.y, sqrtf(z1sq));
        }else{
            float R = sqrtf(cp1.x *cp1.x * r + cp1.y *cp1.y * r);
            float s = 1.0/R;
            p1_z = glm::vec3(s*cp1.x,s*cp1.y, 0);
        }
        if (z2sq>0){
            p2_z = glm::vec3(cp2.x,cp2.y, sqrtf(z2sq));
        }else{
            float R = sqrtf(cp2.x *cp2.x * r + cp2.y *cp2.y * r);
            float s = 1.0/R;
            p2_z = glm::vec3(s*cp2.x,s*cp2.y, 0);
        }
        p1_z = glm::inverse(glm::mat3(viewMatrix)) * p1_z;
        p2_z = glm::inverse(glm::mat3(viewMatrix)) * p2_z;
        p1_z = glm::normalize(p1_z);
        p2_z = glm::normalize(p2_z);
        axis = glm::cross(p1_z,p2_z);
        angle = glm::dot(p1_z,p2_z);
        std::cout<<"angle:"<<angle<<std::endl;
        // std::cout<<axis.x<<" "<<axis.y<<" "<<axis.z<<std::endl;
        axis=glm::normalize(axis);
        if (angle !=1 && glm::length(axis)>0.1){
